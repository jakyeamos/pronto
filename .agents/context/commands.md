...
Repository detail in `repo --json` and a ready scoped `route --json` also
includes `task_lanes` (`pronto-task-lanes/v1`). This is a read-only projection
of signed isolated-change receipts plus live Git bindings. Lease expiry cannot
leave ownership permanent: after the grace period, a verified lane whose
recorded worktree and branch still match live Git is `adoptable`. Dirty WIP is
preserved. Missing, unsigned, or mismatched evidence stays `stale` or `unknown`
and does not authorize mutation. See
[`docs/task-lane-custody.md`](../../docs/task-lane-custody.md).
