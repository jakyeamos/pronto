...
| Findings are unavailable despite fleet audits                   | Repository-local full detector report                                 | Run `pronto quality detector-refresh --json`; inspect every published, blocked, or unsupported QR result and the post-import coverage counts.                 |
