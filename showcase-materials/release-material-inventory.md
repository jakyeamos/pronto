...
The local execution handoff is [`postability-ledger.json`](postability-ledger.json).
It distinguishes a complete local posting packet from a live product/evidence,
authority, hosting, or destination gate. Its `external_posting_proof` field is
false until a fresh destination receipt and readback are captured; a complete
packet is therefore not a claim that the project was posted.

## Inventory totals

- **32** active `public_showcase` repositories are in the release matrix,
  including the 18 DevOps tooling sprint repositories classified on
  2026-08-13.
- **109** current material/gap entries are open across the 32 active targets
  (**41** distinct labels). The 18 new targets are explicitly gated and remain
  non-publishable until their local proof and material packages are complete.
- **147** destination rows belong to active targets (**96 required**, **34
  recommended**, and **17 conditional**). The durable audit matrix contains
  **161** rows in total; the remaining 14 rows are retained as not-applicable
  exclusion history.
...
| Research Domain Writing    | Gated         | gated / gated / gated                   | Local real-case package, source attestation, owner editorial approval, and reviewed 16:9 preview are complete; no-auth hosted verification remains open                                  | DEV.to, daily.dev, LinkedIn planned                             | RDW-5 host the case, verify desktop/mobile, and record readbacks                         |
...
| Portable Agentic Workbench | Gated         | gated / gated / gated                   | Local candidate package now includes case study, claim ledger, short copy, no-auth page source, and reviewed 16:9 binary; permitted host-runtime equivalence, hosted no-auth URL, and external destination readbacks remain open | DEV.to, daily.dev, Hacker News, Reddit, LinkedIn planned        | PW-4 resolve the host-runtime/provider boundary; then verify hosting and distribution readbacks   |
| AI Workflow Leverage       | Excluded      | n/a / n/a / n/a                         | Supporting AL-1/AL-7 packet retained outside the current AI Showcase queue by owner decision                                                                                             | Supporting work only                                             | None — do not include in current queue                                                   |
| Marketing Autoresearch     | Excluded      | n/a / n/a / n/a                         | Supporting MA-1/MA-7 packet retained outside the current AI Showcase queue by owner decision                                                                                             | Supporting work only                                             | None — do not include in current queue                                                   |
| RemodelVision              | Gated         | gated / gated / gated                   | Local synthetic constraint case, scoped owner contribution approval, claim ledger, no-auth source, and reviewed 16:9 binary are complete; current runnable proof, hosted URL, and destination readbacks remain open | Product Hunt, Reddit, LinkedIn planned                          | RV-3 current runtime proof; hosting remains downstream                                      |
| Dsci-proj                  | Gated         | gated / gated / gated                   | Local synthetic two-backlog contract case, claim ledger, no-auth source, and reviewed 16:9 binary are complete; DS-1 adapter/scenario proof, hosted URL, and destination readbacks remain open       | DEV.to, daily.dev, Hacker News, LinkedIn planned                | DS-1 map two backlog schemas into the decision contract                                            |
| Book                       | Gated         | gated / gated / gated                   | Owner-approved synthetic reader-to-media case uses invented text, original copyright-free music, original visuals, claim ledger, no-auth source, and reviewed 16:9 binary; direct reader proof, hosted URL, and readbacks remain open | LinkedIn planned; real Book assets intentionally excluded      | BK-2 author the synthetic transformation arc, then prove the direct reader surface                              |
| Agent Router               | Gated         | gated / gated / gated                   | Local candidate package now includes routing-decision preview, <=500-character description, route-and-receipt case, claim ledger, and no-auth page source; verified comparison outcome, hosted no-auth URL, and external destination readbacks remain open | DEV.to, daily.dev, Hacker News, LinkedIn planned                | AR-5 approve confidence/fallback output contract while preserving native conflict receipt; then verify comparison and distribution readbacks |
| Codex Browser Control      | Gated         | gated / gated / gated                   | Local candidate package now includes synthetic case, claim ledger, short copy, no-auth page source, and reviewed 16:9 binary; installed side-panel plan/apply/verify/stale-refusal receipts, hosted no-auth URL, and external destination readbacks remain open | DEV.to, daily.dev, Hacker News, Reddit, LinkedIn planned        | CB-2/CB-3 reload the matching extension/native-host pair and capture round trip; then verify hosting/readbacks |
...
| Review Attention Map         | W3 review MVP: explicit contract/behavior signals over a diff with source evidence and stale/unmatched visibility. Candidate local packet now includes historical/current-dev two-source overlay, unmatched, and reviewed receipts, claim ledger, crop-safe 1600×900 PNG/SVG preview, short copy, and no-auth case source. | Gated public target; producer/source-freshness readback, direct navigation breadth, negative-state breadth, hosting, and destination readback remain partial. | Close RAM-0/RAM-2 with producer binding, direct navigation, and stale/unmatched/reviewed matrix, then promote to RAM-3.                    |
| Review Sandbox               | W4 disposable-workspace MVP: preview/create/cleanup for repository-declared scenarios with dirty-state retention. Candidate local packet now includes historical/current-dev clean/failed/dirty/cancellation-shaped receipts, claim ledger, crop-safe 1600×900 PNG/SVG preview, short copy, and no-auth case source. | Gated public target; distinct cancellation/process proof, primary-checkout safety, hosting, and destination readback remain partial.       | Close RS-0/RS-2 with distinct cancellation/process proof, primary-checkout safety, and cleanup authority, then promote to RS-3.                    |
| Change Integration Simulator | W4 integration MVP: immutable source/target resolution and merge-tree receipts without ref mutation. Candidate local packet now includes historical/current-dev clean/conflict simulation, separate gate-probe, and ref-safety receipts, claim ledger, crop-safe 1600×900 PNG/SVG preview, short copy, and no-auth case source. | Gated public target; Gateboard handoff, stale/cancellation/dirty breadth, hosting, and destination readback remain partial.                | Close CIS-0/CIS-2 with a real Gateboard receipt, negative matrix, and ref-safety proof, then promote to CIS-3.                                     |
| Deletion Proof Workbench     | W4 proof MVP: bounded symbol inspection, unknown consumer classes, explicit apply, and recovery reference. Current-dev packet includes bounded deletion/refusal/recovery and unknown-class receipts, claim ledger, reviewed 1600×900 PNG/SVG preview, short copy, and no-auth case source. | Gated public target; stale/negative breadth, downstream parity, hosting, and destination readback remain partial. | Close DPW-1/DPW-3 with stale/generated/reflective/failing-test fixtures, Change Radius/Contract Watch parity, and no-auth/external readbacks. |
| Readiness Inspector          | W5 portfolio MVP: goal-specific checks with owner, predicate, outcome, evidence, and next action; no aggregate score. Current-dev packet includes target-bound passed receipts, explicit unsupported/blocked state evidence, claim ledger, reviewed 1600×900 PNG/SVG preview, short copy, and no-auth case source. | Gated public target; upstream projection, full state matrix, native follow-up, hosting, and destination readback remain partial. | Close RI-0/RI-3 with Quality Setup/Evidence Replay projection, state breadth, native follow-up, and no-auth/external readbacks. |
| Fleet Radar                  | W5–W6 portfolio MVP: explicit registry, per-repository freshness/blockers, receipt-contract classification, native-link metadata, broad failure states, and read-only snapshot writing. Current-dev packet includes W5/W6/W8 refresh and contract evidence, a visually inspected 1600×900 PNG, a validated owner-only Sites version, claim ledger, preview source, short copy, and no-auth case source. | Gated public target; two healthy real producer readbacks and direct native navigation remain evidence gates; public access, deployment, credentialless URL readback, and destination readbacks remain packaging gates. | Close FR-0/FR-1 with two real producer receipts and direct native navigation, then confirm public access, deploy the saved case, and capture no-auth/external readbacks for FR-3. |
...
- **ESLint Anti-Slop** is retained as a standalone package but its showcase
  ...
  release card.
- **AI Code Quality Stack** now has a local candidate case, claim ledger,
  reviewed 16:9 preview, short copy, and no-auth page source. Its Pre-CR and
  Quality Runner execution receipts remain open, so the combined story is not
  publishable yet.
